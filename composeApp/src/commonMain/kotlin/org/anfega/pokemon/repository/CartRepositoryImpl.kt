...
class CartRepositoryImpl(
    ...
    override suspend fun getCartItems(): List<Pokemon> = queries.selectAll().executeAsList().map {
        Pokemon(it.id.toInt(), it.name, it.imageUrl, it.price)
    }

    override suspend fun addToCart(pokemon: Pokemon) = queries.insertOrReplace(
    ...
    override suspend fun removeFromCart(pokemonId: Int) = queries.deleteById(pokemonId.toLong())
